...
OUTPUT_PATH = ROOT / 'results' / 'comparison' / 'memetic_vs_sa_ga_hybrid_medium.json'
SOLVER_KEYS = ['simulated_annealing', 'genetic_algorithm', 'memetic_ga_sa', 'hybrid_bsgo_ga']
INSTANCE_NAMES = [
    '3sat_n100_r3.json', '3sat_n100_r4.3.json', '3sat_n100_r6.json',
    '3sat_n150_r3.json', '3sat_n150_r4.3.json', '3sat_n150_r6.json',
    '3sat_n200_r3.json', '3sat_n200_r4.3.json', '3sat_n200_r6.json',
]
BASE_SEED = 101
...
def solver_config(key: str, n: int, seed: int):
    ...
    if key == 'memetic_ga_sa':
        return {
            ...
            'sa_max_iterations': min(2000, max(800, 12 * n)),
            'lamarckian': True,
            'random_seed': seed,
        }
    if key == 'hybrid_bsgo_ga':
        return {
            'population_size': 60,
            'max_iterations': 180 if n == 100 else 210 if n == 150 else 230,
            'crossover_rate': 0.92,
            'mutation_rate': min(0.04, 2.0 / max(n, 1)),
            'c_coefficient': 0.72,
            'w_inertia': 0.72,
...
for instance_name in INSTANCE_NAMES:
    ...
    for offset, key in enumerate(SOLVER_KEYS):
        ...
        per_run.append(row)
        print(f"{instance_name} | {result.algorithm_name} | {result.best_satisfied_clauses}/{result.total_clauses} | {result.runtime_seconds:.3f}s", flush=True)
